...
fn assemble_test_file(
    ...
    assemble_file(
        ...
        verify_cache,
        false,
        crate::game_installer::VerifyMode::Full,
    )
}
...
fn assemble_file_parallel_file_hash_passes() {
    ...
    let chunk_refcounts: Vec<AtomicU32> = (0..chunks.len()).map(|_| AtomicU32::new(1)).collect();
...
fn run_assembly_task_out_of_bounds_file_idx() {
    ...
    let params = AssemblyTaskParams {
        ...
        verify_mode: crate::game_installer::VerifyMode::Full,
    };
...
fn assemble_file_prng_data_matches_expected_md5() {
    ...
    let chunk_data: Vec<Vec<u8>> = (0..4).map(|i| prng_block(42 + i, 16384)).collect();
    ...
    assert_eq!(
        result, expected,
        "assembled output differs from decompressed concatenation"
    );
